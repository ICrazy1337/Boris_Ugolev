...
BEGIN
    SELECT coalesce(s.issued_id, nextval('library.issueds_sq')) AS issued_id,
           s.books,
           s.user_id,
           s.return_date,
           s.is_returned,
    ...
    WHERE book_id IN (SELECT (data ->> 'book_id')::INT FROM jsonb_array_elements(_books) AS data);
    ...
    SELECT _issued_id, (elem ->> 'book_id')::INT, now() AT TIME ZONE 'Europe/Moscow'
